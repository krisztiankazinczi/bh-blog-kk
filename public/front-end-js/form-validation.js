function validateForm(validationValues) {
  let errorObject = {}
  if (validationValues.pw_confirm && !comparePasswords(validationValues.pw, validationValues.pw_confirm)) return errorObject = {'pw_error': 'Please use the same password in the password confirmation field as in password field was used'}
  ...
  for (let [key, value] of Object.entries(validationValues)) {
    ...
    if (!value) errorObject[`${key}`] = `This field is required`
      ...
      if (result.errorMessage) errorObject[`${key}`] = `${result.errorMessage}`  
    } 
  }
  return errorObject;
}
